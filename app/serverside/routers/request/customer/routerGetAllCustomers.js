...
export default function routerGetAllCustomers(req, res) { // this function is taking care of the getAllCustomers request from the user
    if (req.body) // if the body of the request is real and not null then we can begin sending the data to the right function
    {
        let data = req.body;
        mongooseCustomer.getAllCustomers(data.host).then(resp=>{
