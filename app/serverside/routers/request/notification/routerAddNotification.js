...
export default function routerAddNotification(req, res) { // this function is taking care of the addNotification request from the user
    if (req.body) // if the body of the request is real and not null then we can begin sending the data to the right function
    {
        let data = req.body;
        mongooseNotification.addNotification(data).then(resp=>{
