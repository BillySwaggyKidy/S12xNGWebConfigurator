...
// this component represent all of the initials parameters of the fields in a row in the averages P1P2 table
export function OthersAveragesInitialsText({valueFields, handleChangeFunction, readOnly, hitClip = false}) {
    ...
    let arrayOthersAveragesInitialsTextRowFields = [
    ...
    return [
        arrayOthersAveragesInitialsTextRowFields.map((field)=>
            <FormText key={field.id} {...field} readOnly={readOnly} onChange={handleChangeFunction} variant="table" hitClip={hitClip}/>
        ),
    ].flat();
};

// this component represent all of the initials parameters of the fields in a row in the averages card channels table
export function OthersAveragesInitialsCardChannels({valueFields, handleChangeFunction, readOnly, hitClip = false}) {
    const arrayActive = [
    ...
    // this array contain all of the fields data like required, the id, his value as a object
    // the id is really important it represent the path to access the data in the object from the db, each new property or index array is separated with a "."
    let arrayOthersAveragesInitialsCardChannelsRowFields = [
    ...
    return [
        arrayOthersAveragesInitialsCardChannelsRowFields.slice(0,1).map((field)=>
            <FormActiveBox key={field.id} {...field} readOnly={readOnly} onChange={handleChangeFunction} variant="table" hitClip={hitClip}/>
        ),
        arrayOthersAveragesInitialsCardChannelsRowFields.slice(1,2).map((field)=>
            <FormShrink key={field.id} {...field} readOnly={readOnly} onChange={handleChangeFunction} variant="table" hitClip={hitClip}/>
        ),
        arrayOthersAveragesInitialsCardChannelsRowFields.slice(2).map((field)=>
            <FormActiveBox key={field.id} {...field} readOnly={readOnly} onChange={handleChangeFunction} variant="table" hitClip={hitClip}/>
        ),
    ].flat();
};
...
// this component represent all of the initials parameters of the fields in a row in the averages P1P2 table
export function OthersAveragesInitialsP1P2({valueFields, handleChangeFunction, readOnly, hitClip = false}) {
    const arrayUnitType = [
        {value:0,label:"NU"},
        {value:1,label:"H"},
        {value:2,label:"L"},
    ...
    // this array contain all of the fields data like required, the id, his value as a object
    // the id is really important it represent the path to access the data in the object from the db, each new property or index array is separated with a "."
    let arrayOthersAveragesInitialsP1P2RowFields = [
        {id:"p1Parameters.type", required:true, items:arrayUnitType, value:valueFields?.p1Parameters.type, width:85},
        {id:"p1Parameters.value", required:true, value: valueFields?.p1Parameters.value, condition:(value)=>value>=-Number.MAX_VALUE, width:90},
        ...
        {id:"p2Parameters.value", required:true, value: valueFields?.p2Parameters.value, condition:(value)=>value>=-Number.MAX_VALUE, width:90},
    ...
    return Array(2).fill(null).map((value, index)=>{
        const arrayAverages = arrayOthersAveragesInitialsP1P2RowFields.slice(5*index,5*(index+1));
        return [
            arrayAverages.slice(0,1).map((field)=>
                <FormSelect key={field.id} {...field} readOnly={readOnly} onChange={handleChangeFunction} variant="table" hitClip={hitClip}/>
            ),
            arrayAverages.slice(1,3).map((field)=>
                <FormShrink key={field.id} {...field} readOnly={readOnly} onChange={handleChangeFunction} variant="table" hitClip={hitClip}/>
            ),
            arrayAverages.slice(3,4).map((field)=>
                <FormSelect key={field.id} {...field} readOnly={readOnly} onChange={handleChangeFunction} variant="table" hitClip={hitClip}/>
            ),
            arrayAverages.slice(4).map((field)=>
                <FormShrink key={field.id} {...field} readOnly={readOnly} onChange={handleChangeFunction} variant="table" hitClip={hitClip}/>
            ),
        ].flat();
    }).flat();
