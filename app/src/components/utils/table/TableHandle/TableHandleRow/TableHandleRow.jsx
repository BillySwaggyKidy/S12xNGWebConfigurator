...
import { updateObjProp } from "../../../../../../utils/functions-utils.js";
...
export default function TableHandleRow({linkedRef, index, rowName, rowFieldsObject, changeToTable, readOnly = false, generateRowFieldsFunction}) {
    ...
    //         console.log(clipboardRowValue);
    //         if (typeof clipboardRowValue == "object") { // we check that it is an object
    ...
    //console.log(newRowFieldsObject);
    return (
        <TableRow>
            <TableCell tabIndex={-1} className='bg-slate-300 p-0 text-center select-none focus:border-2 focus:border-current'>{rowName}</TableCell>
            {
                // we display all of the TableCell with the fields as content using the generateRowFieldsFunction props
                generateRowFieldsFunction(rowFieldsObject, handleRowFieldChange, readOnly).map((field, index)=>
